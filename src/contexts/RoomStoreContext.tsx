...
export const RoomStoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
          ...
          if (window.showSaveFilePicker) {
            try {
              ...
              toast({
                ...
                description: "No se pudo guardar en archivo. Los datos se guardaron en almacenamiento local del navegador.",
                variant: "destructive",
              });
            }
          } else {
  ...
  const updateRoom = (updatedRoom: Room) => {
    ...
    setRooms(
      rooms.map(room => (room.id === updatedRoom.id ? roomWithTimestamp : room))
    );
  };
